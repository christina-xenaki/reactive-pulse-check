...
5. **Any notes attached by the answers given** (`noteId`, section C.4), in their own block immediately under the recommendation. Separate from item 4, which is for overrides that beat the arithmetic, and from item 6, which is for the check-yourself flag: a note is neither of those, and rendering it inside either block would misstate what produced it. One note is conditional on the final level rather than on the answer alone: where employees are already discussing it (`q8.a`), the record attaches the internal-audience note (`rule.internalAudienceNote`) — the ladder describes external response only, and staff speculating without facts is how an internal rumour becomes an external story. That is true whatever level the recommendation lands at, so the note fires at every level, not only a low one: an organisation issuing a reactive statement to a journalist still has staff speculating, and still needs to address them. The note is computed in `js/overrides.js`, alongside the check-yourself flag and the Q9 legal cross-check, because it needs the final level, which is only known once overrides have been resolved.

   **Fixed bug: the note's level condition was hardcoded, not config-driven.** From when the note was first added until this fix, `js/overrides.js` carried a literal `finalLevel !== 1 && finalLevel !== 2` check, restricting the note to Levels 1 and 2 — a threshold written directly into code rather than as a config value, contrary to CLAUDE.md's config-not-code principle. It has been replaced with `bandBoundaries.internalAudienceNoteMaxLevel`, a config value currently set to 7 (the top of the scale), so the restriction can be tightened again from config alone if that is ever wanted, without touching code.
