...
**Fixed bug: "the path actually taken" was not actually enforced.** From when scoring and Back-navigation first coexisted until the stale-answer fix (a dedicated diagnosis-and-fix session), the paragraph above already described the intended behaviour, and `js/scoring.js` carried a comment citing this section for it, but the implementation did not match either: `axisTotals()`, `driversFor()` and `scoredSelections()` (and, in `js/render.js`, `buildAnswerRecord()`) all read every key ever set in the `answers` object, not the questions on the path actually taken. Using Back to change an early answer (e.g. switching Q1's issue type after already answering that type's branch questions) left the abandoned branch's answers in place; they kept scoring, kept feeding the drivers list and the low-confidence count, and kept appearing in the exported answer record, alongside the new branch — silently, since nothing marked the result as affected. Both the earned points and the maximum they were normalised against were inflated the same way, so this was not a numerator-only drift: the percentage itself, and sometimes the recommended level, could be wrong. The fix (`PulseCheck.Scoring.questionsOnPath()`, the one shared definition of "questions on the current path" that every answer-reading function now goes through) also made `js/questions.js` discard an answer as soon as its question falls off the current path, rather than only filtering it out at scoring/render time — so switching back to a previously abandoned branch means answering it again, with no memory of the abandoned answers anywhere in the tool. See the PATH-SCOPED comments in `js/scoring.js`, `js/questions.js`, `js/overrides.js` and `js/render.js`.
