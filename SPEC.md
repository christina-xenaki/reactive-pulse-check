...
5. **Any notes attached by the answers given** (`noteId`, section C.4), in their own block immediately under the recommendation. Separate from item 4, which is for overrides that beat the arithmetic, and from item 6, which is for the check-yourself flag: a note is neither of those, and rendering it inside either block would misstate what produced it. One note, the internal-audience note (`rule.internalAudienceNote`), attaches whenever employees are already discussing it (`q8.a`), at every level — the note says the recommendation covers external response only, and that is true regardless of level: an organisation issuing a reactive statement to a journalist at Level 4 still has staff speculating who need addressing. Its firing condition (which option triggers it, and which levels it is restricted to, if any) is a config value, `config.noteRules['rule.internalAudienceNote']`, read by `js/overrides.js` alongside the check-yourself flag and the Q9 legal cross-check — it is computed there, rather than in `js/scoring.js`, because it needs the final level, which is only known once overrides have been resolved.

    **Correction, 2026-08-30.** This note originally fired only where the recommendation landed at Level 1 or 2 — a level check hardcoded as a plain JS condition in `js/overrides.js`, which is not what this document's config-not-code principle allows for a threshold (see CLAUDE.md, "Config, not code, for content"). It has moved to config, `config.noteRules['rule.internalAudienceNote'].levels`, now set to `null` (no restriction), so the note fires at every level. Reasoning: the note's own wording says the recommendation covers external response only, which holds however high the level runs — the Level 1/2 restriction excluded the case the note exists for. The wording was checked against every level and needs no change; it never named one.
